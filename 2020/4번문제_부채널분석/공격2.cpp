...
//  1열 2열 3열 4열
//
// 공격2 코드에서는 9라운드의 2열을 공격함.
// 따라서 암호문 C에서의 결과1의 2열 04, 01, 14, 11을 공격함.
// 10라운드 키 key(10,14) = z ( z는 0~255)
// 9라운드 키  e = 0xCB, f는 0~255, g, h =0으로 가정하여 전력 파형의 정보를 얻기 위한 공격임.
...
int main(void) {
	...
	// key(10,14) = a, key(9,4) = 0xCB, key(9,5)= b, key(9,6), key(9,7) = 0
	for (int a = 0; a < 256; a++) {
		for (int e = 0; e < 1025; e++) {
			// 10라운드 InvKeyAddtion 	
			AK_10[e][THIRD] = a ^ CipherText[e][THIRD];
		...
		for (int b = 0; b < 256; b++) {
			...
			for (int e = 0; e < 1025; e++) {
				// 9라운드 InvKeyAddtion
				AK_9[e][FIRST] = 0xCB ^ SB_10[e][FIRST];
				AK_9[e][SECOND] = b ^ SB_10[e][SECOND];
			...
			for (int k = 0; k < 4; k++) {
				...
				for (int i = 0; i < 10000; i++) {
					...
					for (int e = 0; e < 1025; e++) {
						...
						sigma1 += z * z;
						sigma2 += z * (double)(PowerTraces[10000 + i][e] - aver_p[10000 + i]);
					}
					P = sigma2 / sqrt(sigma1) / sqrt(sigma3[10000 + i]);
	...
	printf("key(10,14) = %d일 때 아래와 같은 결과를 얻음\n", aaa[0]);
	for (int i = 0; i < 4; i++) {
